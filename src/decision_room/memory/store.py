...
class RoomMemoryStore:
    ...
    def apply_journal_event(self, payload: Any, room_id: str) -> None:
        """Apply a ``memory.write`` journal-event payload to local state.

        Idempotent for fact writes (same key+value yields no observable
        change). Event records ARE appended unconditionally, so replaying
        the journal cold-starts the full event stream. Designed so the
        ``RoomEventJournal`` remains the single source of truth and this
        store is a denormalized projection that can be regenerated by
        replaying the journal through ``RoomProjector.apply()``.
        """
        ...
        if isinstance(fact_key, str) and fact_key.strip():
            self.write_fact(room_id, scope, fact_key.strip(), payload.get("fact_value"))
        event_type = payload.get("memory_event_type")
        if isinstance(event_type, str) and event_type.strip():
            event_payload = payload.get("memory_event_payload")
            self.record_event(
                room_id,
                scope,
                event_type.strip(),
